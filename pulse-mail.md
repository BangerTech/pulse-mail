...
Aktuelle Version: **1.4.1** — Quelle ist `frontend/package.json`, Anzeige unter Einstellungen → Info (`__APP_VERSION__` / `__APP_BUILD__` aus dem Vite-Build). Fallback in `frontend/src/shared/version.ts`. Backend `package.json` und `desktop/` halten dieselbe Versionsnummer.
...
#### 1.4.1 (2026-09-12)
- GitHub-Workflow **Windows App** startet bei Push auf `main` (vorher nur bei Tags `v*`, darum lief er nicht)
...
2. Nach dem Lauf unter Artifacts den NSIS-Installer herunterladen (`Pulse Mail_1.4.1_x64-setup.exe` o. ä.)
...
- Version aus `frontend/package.json` (aktuell **1.4.1**), Build-Zeitpunkt aus dem Vite-Build (`__APP_VERSION__`, `__APP_BUILD__`)
- Hinweis-Berechtigung und Tonkanal-Status. Titelzeile der Einstellungen zeigt `v1.4.1`
